...
func (a *app) newUploadCommandWithUse(use string, short string) *cobra.Command {
	...
	c.Flags().BoolVarP(&opts.Recursive, "recursive", "r", false, "Transfer directories recursively")
	c.Flags().BoolVarP(&opts.Force, "force", "f", false, "Overwrite existing files")
	c.Flags().BoolVarP(&quiet, "quiet", "q", false, "Only print errors")
	return c
}
